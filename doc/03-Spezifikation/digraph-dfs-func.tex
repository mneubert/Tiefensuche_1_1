...
Im Gegensatz zu den vorangegangenen Kapiteln \ref{sec:digraph-dfs-proc} bis
\ref{sec:dfs-rek-1}, bei denen die Tiefensuche als Prozedur mit
Wahrheitswertrückgabe spezifiziert und implementiert wurde, wird nun der
Algorithmus als Funktion umgesetzt. Das heißt, dass sowohl die iterative als
auch die rekursive Variante einen Rückgabeparameter besitzen, der einen Pfad
vom Startknoten a in die Zielknotenmenge z enthält, wenn dieser im Graphen g
existiert. Andernfalls wird die leere Liste zurückgegeben.
\par
...
welche in Abschnitt \ref{sec:digraph-path} erläutert wird. Innerhalb von
...
entsprechend der obigen Ordnung erklärt:

\begin{itemize}
  \item Das erste Axiom besagt, dass die Funktion einen leeren Pfad, d.h. eine
  leere Liste, zurückgibt, wenn ihr entweder eine leere Zielknotenmenge oder
  kein Startelement übergeben wurde.
  ...
  Rückgabepfad der Pfad x.
  \item Axiom 3 und 4 erfüllen beide die Voraussetzung, dass es eine Kante von
  a nach b im Graphen gibt, mit a als Startknoten und b als Zielknoten. Für
  Axiom 3 gilt dann, dass der Start des Rückgabepfades Knoten a ist und in Axiom
  4 das Ende Pfades Knoten b ist.
  \item Axiom fünf bildet die Transitivität ab. Unter der Voraussetzung, dass c
  ein Zielknoten ist und ein Pfad von a nach b und von b nach c existiert, ist
  der Knoten a das erste Element auf dem Rückgabepfad.
